...
double *get_y(NETWORK *n, int layer_nr, double *samples)
{
    LAYER *current_layer = n->l+layer_nr;
    int is_current_layer_last_layer = (n->num_layers == layer_nr + 1);
    int y_size = current_layer->num_neurons;

    if(!is_current_layer_last_layer) // add bias node
        y_size++;

    double *z = get_z(n, layer_nr, samples);
